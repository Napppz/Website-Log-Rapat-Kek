...
import { generateReportPdf } from '@/lib/report/report-pdf-generator';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawFilename = `Laporan-Berkala${biroSlug}-${periodSlug}-${todayWibIso}.pdf`;
    ...
    return NextResponse.json(
      { error: error?.message || 'Gagal menghasilkan dokumen PDF laporan berkala.' },
      { status: 500 }
    );
  }
}
